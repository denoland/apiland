...
Deno.test({
  ...
  async fn() {
    ...
    const [, gv1] = await lookup("std", "0.175.0");
    assert(gv1);
    performance.mark("gv-gv1");
    const [, gv2] = await lookup("std", "0.175.0");
...
Deno.test({
  ...
  async fn() {
    ...
    const [, , me1] = await lookup("std", "0.175.0", "/");
    assert(me1);
    performance.mark("me-me1");
    const [, , me2] = await lookup("std", "0.175.0", "/");
...
Deno.test({
  ...
  async fn() {
    ...
    const dp1 = await lookupDocPage("std", "0.176.0", "/", "$$root$$");
    assert(dp1);
    performance.mark("dp-dp1");
    const dp2 = await lookupDocPage("std", "0.176.0", "/", "$$root$$");
