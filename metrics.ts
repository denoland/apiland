...
async function moduleMetrics() {
  ...
  for (const row of currentStdRows) {
    if (row.dimensions?.[0] && row.metrics?.[0].values) {
      ...
      if (match) {
        ...
        submoduleMetrics[submod].users += parseInt(
  ...
  for (
    ...
    objectSetKey(subModMetrics, {
      path: [{ kind: "submodule_metrics", name }],
    });
...
const MOD_PATH_RE = /^\/(\S+)@(\S+)\/$/;
const STD_PATH_RE = /^\/std@(\d{1,3}\.\d{1,3}\.\d{1,3})\/$/;
const INVALID_KEY_RE = /__.*__/;
...
function createDailyArray(length = 60): UsageMetric[] {
  const arr: UsageMetric[] = [];
  for (let i = 0; i < length; i++) {
    arr.push({ users: 0, sessions: 0 });
  }
  return arr;
}

async function usageMetrics() {
  dax.logStep("Running daily metrics...");
  const [xRows, stdRows] = await runReports({
    viewId: "253817008",
    pageSize: 100_000,
  ...
  dax.logLight(`  retrieved ${xRows.length + stdRows.length} rows of data.`);

  dax.logStep("Calculating usage...");

  const updated = new Date();

  const xUsage = {
    total: { sessions: 0, users: 0 },
    daily: createDailyArray(),
    updated,
  };
  const metrics = new Map<string, ModuleUsage>();
  for (const row of xRows) {
    assert(row.dimensions);
    assert(row.metrics);
    const [dayStr, path] = row.dimensions;
    const day = parseInt(dayStr, 10);
    const [sessionsStr, usersStr] = row.metrics[0].values!;
    const sessions = parseInt(sessionsStr, 10);
    const users = parseInt(usersStr, 10);
    const match = MOD_PATH_RE.exec(path);
    if (match) {
      const [, mod, ver] = match;
      if (INVALID_KEY_RE.test(mod) || INVALID_KEY_RE.test(ver)) {
        continue;
      }
      xUsage.total.sessions += sessions;
      xUsage.total.users += users;
      xUsage.daily[day].sessions += sessions;
      xUsage.daily[day].users += users;
      if (!metrics.has(mod)) {
        metrics.set(mod, {
          total: { users: 0, sessions: 0 },
          daily: createDailyArray(),
          versions: {},
          updated,
        });
      }
      const metric = metrics.get(mod)!;
      metric.total.sessions += sessions;
      metric.total.users += users;
      metric.daily[day].sessions += sessions;
      metric.daily[day].users += users;
      if (!(ver in metric.versions)) {
        metric.versions[ver] = { sessions: 0, users: 0 };
      }
      metric.versions[ver].sessions += sessions;
      metric.versions[ver].users += users;
    }
  }
  ...
  const stdUsage = metrics.get("std")!;
  for (const row of stdRows) {
    ...
    const sessions = parseInt(sessionsStr, 10);
    const users = parseInt(usersStr, 10);
    const match = STD_PATH_RE.exec(path);
    if (match) {
      const [, ver] = match;
      if (INVALID_KEY_RE.test(ver)) {
        continue;
      }
      stdUsage.total.sessions += sessions;
      stdUsage.total.users += users;
      stdUsage.daily[day].sessions += sessions;
      stdUsage.daily[day].users += users;
      if (!(ver in stdUsage.versions)) {
        stdUsage.versions[ver] = { sessions: 0, users: 0 };
      }
      stdUsage.versions[ver].sessions += sessions;
      stdUsage.versions[ver].users += users;
    }
  }

  const mutations: Mutation[] = [];
  const datastore = new Datastore(keys);
  objectSetKey(xUsage, datastore.key(["metric_usage", "$$x$$"]));
  mutations.push({ upsert: objectToEntity(xUsage) });
  for (const [key, metric] of metrics) {
    objectSetKey(metric, datastore.key(["metric_usage", key]));
    mutations.push({ upsert: objectToEntity(metric) });
  }
